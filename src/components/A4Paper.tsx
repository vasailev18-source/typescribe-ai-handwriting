            ...
            <g key={lineIdx}>
              {line.elements.map((el, elIdx) => {
                    ...
                    row.forEach((cellText, cIdx) => {
                      ...
                      cellText.split('').forEach((char, charIdx) => {
                        ...
                        if (rendering.useFont) {
                            ...
                            <g key={`cell-font-g-${rIdx}-${cIdx}-${charIdx}`}>
                              {config.textOutlineColor && (
                                <text
                                  ...
                                  strokeLinejoin="round"
                                  opacity={0.85}
                                  transform={`rotate(${rendering.rotation || 0}, ${curCharX}, ${rendering.lastPoint[1]})`}
                                >
                                  {char}
                                </text>
                              )}
                          ...
                          const isMultiply = config.inkColor === 'pencil' || config.inkColor === 'marker-yellow' || config.inkColor.startsWith('felt');
                          cellTexts.push(
                ...
                if (el.useFont) {
                  ...
                  const lineStroke = config.inkColor === 'pencil' ? '#6b7280' : inkColorHex;
